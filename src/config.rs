...
pub struct Config {
    ...
    pub tokenizer_profile: &'static str,
...
pub fn reject_legacy_env(keys: impl Iterator<Item = String>) -> Result<(), ConfigError> {
    ...
    Err(ConfigError::LegacyEnv(legacy))
}
...
impl Config {
    ...
    pub fn from_env() -> Result<Self, ConfigError> {
        reject_legacy_env(env::vars().map(|(key, _)| key))?;
    ...
    pub fn from_lookup(mut get: impl FnMut(&str) -> Option<String>) -> Result<Self, ConfigError> {
        ...
        Ok(Self {
            ...
            chat_template_path: tokenizer.chat_template_path,
...
mod tests {
    ...
    #[test]
    fn a_retired_ds4_setting_refuses_to_start() {
        // The rename is a hard cut, so a leftover DS4_ variable would be inert
    ...
    fn defaults_match_go_contract() {
        ...
        assert_eq!(config.tokenizer_profile, "deepseek-v4-r34");
