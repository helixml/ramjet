...
impl SnapshotCompanionConfig {
    ...
    pub fn from_lookup(
        ...
        let mode = match get("DS4_SNAPSHOT_COMPANION_MODE")
            .as_deref()
            .unwrap_or("off")
        {
            "off" => SnapshotCompanionMode::Off,
            "serve" => SnapshotCompanionMode::Serve,
            _ => return Err(invalid("DS4_SNAPSHOT_COMPANION_MODE", "off or serve")),
        };

        let max_clients = bounded_usize(&mut get, "DS4_SNAPSHOT_MAX_CLIENTS", 2, 1, MAX_CLIENTS)?;
        let tail_queue_capacity = bounded_usize(
            &mut get,
            "DS4_SNAPSHOT_TAIL_QUEUE_CAPACITY",
            1_024,
            1,
            MAX_QUEUE_CAPACITY,
        )?;
        ...
        let companion_uid = required_non_root_uid(&mut get, "DS4_SNAPSHOT_COMPANION_UID")?;
        let client_uid = required_non_root_uid(&mut get, "DS4_SNAPSHOT_CLIENT_UID")?;
        ...
        let live = endpoint_list(&mut get, "DS4_SNAPSHOT_LIVE_ENDPOINTS")?;
        let replay = endpoint_list(&mut get, "DS4_SNAPSHOT_REPLAY_ENDPOINTS")?;
        ...
        let event_topic = get("DS4_SNAPSHOT_EVENT_TOPIC").unwrap_or_default();
        if event_topic.len() > MAX_TOPIC_BYTES {
            return Err(invalid("DS4_SNAPSHOT_EVENT_TOPIC", "at most 256 bytes"));
        }
        let sources = live
            .into_iter()
            .zip(replay)
...
mod tests {
    ...
    fn rejects_relative_or_non_normalized_paths_without_echoing_them() {
        for path in [
            ...
            let mut values = configured();
            values.insert("DS4_SNAPSHOT_SOCKET_PATH", path);
    ...
    fn rejects_root_equal_or_malformed_uids() {
        for (key, value) in [
            ("DS4_SNAPSHOT_COMPANION_UID", "0"),
            ("DS4_SNAPSHOT_CLIENT_UID", "nope"),
            ("DS4_SNAPSHOT_CLIENT_UID", "12001"),
        ] {
            let mut values = configured();
    ...
    fn endpoint_cardinality_and_shapes_are_exact() {
        for (key, value) in [
            ("DS4_SNAPSHOT_REPLAY_ENDPOINTS", "tcp://engine-a:5558"),
            (
                "DS4_SNAPSHOT_LIVE_ENDPOINTS",
                "tcp://a:1,tcp://b:2,tcp://c:3",
            ),
            ("DS4_SNAPSHOT_LIVE_ENDPOINTS", "ipc:///tmp/events"),
            (
                "DS4_SNAPSHOT_REPLAY_ENDPOINTS",
