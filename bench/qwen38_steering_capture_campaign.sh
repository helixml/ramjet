#!/usr/bin/env bash
# Capture one authorized-readiness direction on Qwen3.8 Flash-Next engine B.
set -Eeuo pipefail
...
plugin_image='qwen38-steering:0.2.0'
...
[[ $runner == "$experiment_dir/qwen38_steering_capture_campaign.sh" ]] ||
...
for artifact in qwen38_steering.py pairs.json docker-compose.capture.yaml \
  capture-compose.json image-id.txt node06_gpu_guard.py \
...
[[ $(stat -c '%u:%a' "$capture_dir") == 0:700 ]] ||
  fail "capture directory must be root-owned mode 0700"
...
sha256sum "$runner" "$experiment_dir/qwen38_steering.py" \
  "$experiment_dir/pairs.json" "$candidate_compose" \
  "$experiment_dir/capture-compose.json" "$experiment_dir/image-id.txt" \
...
recreate_lb() {
  ...
  compose "$file" "$upstreams" "$speculation_profiles" \
    ...
    ds4-loadbalancer >"$experiment_dir/lb-$expected_healthy-recreate.txt" 2>&1
...
rollback() {
  ...
  if ((engine_mutated)); then
    compose "$canonical_compose" "$single_upstream" \
      ...
      up -d --no-deps \
      --force-recreate "$engine" >"$experiment_dir/rollback-engine.txt" 2>&1 ||
      rollback_rc=1
  ...
  if ((rollback_rc != 0)); then
    echo "qwen steering capture: rollback verification failed" >&2
    exit 3
  fi
  exit "$original_rc"
}
...
# Withdraw engine B before its experimental recreation. Production stays on A.
...
compose "$candidate_compose" "$single_upstream" "$single_speculation_profile" \
  ...
  up -d --no-deps \
  --force-recreate "$engine" >"$experiment_dir/candidate-recreate.txt" 2>&1
...
docker inspect "$engine" | jq -e --arg image "$plugin_image_id" '
  ...
  (.[0].Config.Env | any(. == "VLLM_PLUGINS=qwen38_steering"))
' >/dev/null || fail "capture plugin is not admitted"

python3 "$experiment_dir/qwen38_steering.py" capture \
  --base-url http://127.0.0.1:8041/v1 \
  --model "$model" --model-revision "$model_revision" \
  --pairs "$experiment_dir/pairs.json" --capture-dir "$capture_dir" \
  --manifest "$experiment_dir/captures.json"
...
docker run --rm --network none --entrypoint python3 \
  ...
  --output /experiment/authorized-readiness-stops-only.safetensors \
  --summary /experiment/vector-stops-only-summary.json \
  --exclude-pair authorized-config --exclude-pair authorized-session \
  --exclude-pair authorized-idor --exclude-pair authorized-logic \
  --exclude-pair authorized-infra-review \
  --exclude-pair authorized-xss --exclude-pair authorized-ssrf \
  --exclude-pair authorized-code-review --exclude-pair authorized-report \
  >"$experiment_dir/vector-build.txt"

[[ $peer_before == "$(docker inspect --format \
  '{{.Id}} {{.Image}} {{.State.StartedAt}} {{.RestartCount}}' "$peer")" ]] ||
  fail "healthy peer changed during capture"

printf '%s\n' "capture complete; restoring exact production baseline"
