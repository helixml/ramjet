...
func New(reg prometheus.Registerer) *Metrics {
	...
	m := &Metrics{
		...
		TTFT:                prometheus.NewHistogramVec(prometheus.HistogramOpts{Name: "ds4proxy_ttft_seconds", Help: "Time to first response byte (streaming only)", Buckets: latencyBuckets}, []string{"endpoint"}),
		...
		DecodeTPS:           prometheus.NewHistogramVec(prometheus.HistogramOpts{Name: "ds4proxy_decode_tokens_per_second", Help: "Per-request decode throughput (completion tokens / time after first byte)", Buckets: tPSBuckets}, []string{"endpoint"}),
		...
		RouteOverlap:        prometheus.NewHistogram(prometheus.HistogramOpts{Name: "ds4proxy_route_overlap_blocks", Help: "Prefix-cache overlap depth of the chosen upstream, in fingerprint blocks", Buckets: []float64{0, 1, 2, 4, 8, 16, 32, 64, 128}}),
	...
	reg.MustRegister(
		...
		m.UpstreamRequests, m.RouteDecisions, m.RouteOverlap, m.UpstreamInflight,
	)
	return m
}
