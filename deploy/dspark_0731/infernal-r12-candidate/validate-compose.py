#!/usr/bin/env python3
"""Prove the r12 qualification render isolates production from candidate B."""
...
CANDIDATE_IMAGE = (
    "voipmonitor/vllm:infernal-invocation-vllmdc2934e-"
    "b12xd48c62b-fi1ac6942-cu133-torch213-20260814-r12@"
...
EXTRA_VOLUMES = [
    {
        "type": "bind",
        "source": "/prod/engine-cache/infernal-invocation-cu133-r12",
        "target": "/cache",
        "bind": {},
    },
    {
        "type": "bind",
        "source": "/prod/engine-cache/infernal-invocation-cu133-r12/tmp",
...
def render(*, candidate: bool) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        fail("docker compose could not render Infernal r12 profile")
    try:
...
def validate(base: dict[str, Any], candidate: dict[str, Any]) -> None:
    ...
    if set(services) != set(base_services):
        fail("r12 overlay changes the service set")
    if services.get("dspark-0731") != base_services.get("dspark-0731"):
        fail("r12 overlay changes engine A")
    ...
    if services.get("ds4-loadbalancer") != expected_lb:
        fail("r12 overlay does not exactly single-home the load balancer")
    ...
    if candidate != expected_document:
        fail("r12 overlay changes unrelated top-level Compose settings")
...
def main() -> int:
    try:
        ...
        print(str(error))
        return 1
    print("Infernal r12 Compose validation passed: LB on A, candidate on B GPUs 4-7")
    return 0
